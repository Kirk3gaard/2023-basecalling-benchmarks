...
NP_QC_combined<-bind_rows(NP_QC_fast,NP_QC_hac)
```

## NP reads aligned to the refs

```{r fig.width=10,fig.height=7,echo=FALSE,warning=FALSE,message=FALSE}
NP_QC_combined %>% ggplot(aes(x = percentIdentity,col=REF))+geom_density()+geom_vline(xintercept = 99,col="red",linetype="dashed")+scale_x_continuous(breaks = 90:100,limits = c(90,100))+facet_wrap(~model,ncol = 1)
```
